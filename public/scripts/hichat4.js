    ...
    init: function() {
        ...
        this.socket.on('connect', function() {
            ...
            $.get('/username', function(data){
                ...
                that.socket.emit('login4', nickName);
            })
        ...
        this.socket.on('loginSuccess4', function() {
        ...
        this.socket.on('system4', function(nickName, userCount, type) {
            var msg = nickName + (type == 'login4' ? ' joined' : ' left');
            that._displayNewMsg('system ', msg, 'red');
            document.getElementById('status').textContent = userCount + (userCount > 1 ? ' users' : ' user') + ' online';
        });
        this.socket.on('newMsg4', function(user, msg, color) {
            that._displayNewMsg(user, msg, color);
        });
        this.socket.on('newImg4', function(user, img, color) {
        ...
        document.getElementById('sendBtn1').addEventListener('click', function() {
            ...
            if (msg.trim().length != 0) {
                that.socket.emit('postMsg4', msg, color);
        ...
        document.getElementById('sendImage1').addEventListener('change', function() {
            if (this.files.length != 0) {
                ...
                reader.onload = function(e) {
                    this.value = '';
                    that.socket.emit('img4', e.target.result, color);
