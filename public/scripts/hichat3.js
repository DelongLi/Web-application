    ...
    init: function() {
        ...
        this.socket.on('connect', function() {
            ...
            $.get('/username', function(data){
                ...
                that.socket.emit('login3', nickName);
            })
        ...
        this.socket.on('loginSuccess3', function() {
        ...
        this.socket.on('system3', function(nickName, userCount, type) {
            var msg = nickName + (type == 'login3' ? ' joined' : ' left');
            that._displayNewMsg('system ', msg, 'red');
            document.getElementById('status').textContent = userCount + (userCount > 1 ? ' users' : ' user') + ' online';
        });
        this.socket.on('newMsg3', function(user, msg, color) {
            that._displayNewMsg(user, msg, color);
        });
        this.socket.on('newImg3', function(user, img, color) {
        ...
        document.getElementById('sendBtn1').addEventListener('click', function() {
            ...
            if (msg.trim().length != 0) {
                that.socket.emit('postMsg3', msg, color);
        ...
        document.getElementById('sendImage1').addEventListener('change', function() {
            if (this.files.length != 0) {
                ...
                reader.onload = function(e) {
                    this.value = '';
                    that.socket.emit('img3', e.target.result, color);
