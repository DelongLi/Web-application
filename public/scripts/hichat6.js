    ...
    init: function() {
        ...
        this.socket.on('connect', function() {
            ...
            $.get('/username', function(data){
                ...
                that.socket.emit('login6', nickName);
            })
        ...
        this.socket.on('loginSuccess6', function() {
        ...
        this.socket.on('system6', function(nickName, userCount, type) {
            var msg = nickName + (type == 'login6' ? ' joined' : ' left');
            that._displayNewMsg('system ', msg, 'red');
            document.getElementById('status').textContent = userCount + (userCount > 1 ? ' users' : ' user') + ' online';
        });
        this.socket.on('newMsg6', function(user, msg, color) {
            that._displayNewMsg(user, msg, color);
        });
        this.socket.on('newImg6', function(user, img, color) {
        ...
        document.getElementById('sendBtn1').addEventListener('click', function() {
            ...
            if (msg.trim().length != 0) {
                that.socket.emit('postMsg6', msg, color);
        ...
        document.getElementById('sendImage1').addEventListener('change', function() {
            if (this.files.length != 0) {
                ...
                reader.onload = function(e) {
                    this.value = '';
                    that.socket.emit('img6', e.target.result, color);
